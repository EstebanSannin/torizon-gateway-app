...
- **Kernel & Connectivity cards** — a shared span-2 pattern: `.k-rel` (mono headline) + `.k-badge` chips, an optional `.k-sub` subtitle, a `.k-kv` definition grid, and a `.k-foot`/`.k-raw` footer. Kernel: release + arch/SMP/PREEMPT + toolchain/binutils/build/date + raw `/proc/version`. Connectivity: IP + `/prefix` + WAN + `.wifi-pill` connected, subtitle iface·type·speed, grid gateway/DNS/method/MAC/MTU, and a footer of other live uplinks.
