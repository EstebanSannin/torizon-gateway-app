...
| **Automated testing + CI** — no automated tests yet; correctness rests on `go vet` + manual on-device validation. Plan: table-driven **unit tests** for the pure-Go parsers first (real device fixtures — `/proc/version`→KernelInfo, CAN netlink attrs, CPU SoC-id/util, Wi-Fi security/band-channel, connInfo/default-route), then handler/template tests (httptest + mock data, auth/CSRF), an on-device smoke checklist, and a **GitHub Actions** workflow (gofmt/vet/test + multi-arch build). | Manual testing by colleagues in the near term (eval via the published image + root compose); build the harness before GA. | Before GA |
